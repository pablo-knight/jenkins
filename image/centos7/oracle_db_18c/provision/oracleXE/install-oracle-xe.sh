...
#        fi
        ...
        if [ -f "/vagrant/oracle/oracle-database-xe-18c-1.0-1.x86_64.rpm" ]
            then
                echo "Install Oracle database..."
                sudo yum -y localinstall /vagrant/oracle/oracle-database*18c*
            else
                echo "oracle-database-xe-18c-1.0-1.x86_64.rpm not available for installation. Please check path where oracle RPM is available."
        fi
fi
...
if [ `grep -En "^export ORACLE_SID=.*$" /home/$ORACLE_USER/.profile | wc -l` -eq 0 ]
    then
        su -l $ORACLE_USER -c 'echo "export ORACLE_SID=PI" | tee -a ~/.profile'
fi
...
if [ -d "/opt/oracle/product/18c/dbhomeXE" ] && [ ! -d "/opt/oracle/oradata/XE" ]
   then
        ...
        sed -i  "s/ORACLE_SID=XE/ORACLE_SID=PI/g" /etc/init.d/oracle-xe-18c && \
        sed -i  "s/DEFAULT_SERVICE=XE/DEFAULT_SERVICE=PI/g" /etc/init.d/oracle-xe-18c 
...
create user pi identified by TEST1234;
grant all privileges to pi CONTAINER=ALL;
quit:
EOF'
